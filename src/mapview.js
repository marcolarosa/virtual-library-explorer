...
const CAMERA_INIT = { x: 20, y: 110, z: 200 };
...
export function initMap(cont) {
    ...
    const planeMat = new THREE.MeshBasicMaterial({ color: "light blue" });
...
function _createCard(node, sourceColor, sourceId, faceAngle) {
  ...
  `;
    ...
    srcLabel.style.cssText =
        "color: #5a7090; font-size: 10px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;";
    const source = SOURCES.find((s) => s.id === sourceId);
